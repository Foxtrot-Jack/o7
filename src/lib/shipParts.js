...
export function createEmptyDesign() {
  ...
  for (const slot of SHIP_SLOTS) {
    if (slot.category === 'hull') parts[slot.id] = { partId: 'hull_arrow', scale: [1, 1, 1] };
    else if (slot.category === 'cockpit') parts[slot.id] = { partId: 'cockpit_bubble', scale: [1, 1, 1] };
    else if (slot.category === 'wing') parts[slot.id] = { partId: 'wing_straight', scale: [1, 1, 1] };
    else if (slot.category === 'engine') parts[slot.id] = { partId: 'engine_basic', scale: [1, 1, 1] };
    else parts[slot.id] = { partId: null, scale: [1, 1, 1] };
  }
