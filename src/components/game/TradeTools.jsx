// Trade Tools — container with Inara-style trade finder and Spansh-style route plotter
import React, { useState } from 'react';
import { TrendingUp, Route } from 'lucide-react';
import InaraTrade from './InaraTrade';
import RoutePlotter from './RoutePlotter';

export default function TradeTools() {
  const [tab, setTab] = useState('inara');
  return (
    <div className="w-full h-full flex flex-col">
      <div className="flex gap-1 border-b border-orange-900/50 p-1">
        <button onClick={() => setTab('inara')} className={`flex items-center gap-1.5 px-3 py-1.5 text-xs border transition-all ${tab === 'inara' ? 'border-orange-500 bg-orange-950/40 text-orange-300' : 'border-transparent text-orange-700 hover:text-orange-500'}`}>
          <TrendingUp className="w-3.5 h-3.5" /> Inara
        </button>
