...
// Finds best buy/sell opportunities between current system and nearby systems
import React, { useState, useEffect, useMemo } from 'react';
import { useGameState } from '@/lib/gameState';
import { COMMODITIES, ECONOMY_TYPES } from '@/lib/commodities';
...
export default function TradeRoutes() {
  ...
  const computeRoutes = useMemo(() => () => {
    setLoading(true);
    // Use setTimeout to avoid blocking UI during system generation
    setTimeout(() => {
      const center = state.currentSystem;
      const stars = generateStarsInRange(center.x, center.y, center.z, 80);
      ...
      for (const star of populated) {
        ...
        // Forward route: current produces → target consumes
        ...
        // Return route: target produces → current consumes
      ...
      // Sort by profit/distance ratio
      opps.sort((a, b) => (b.profit / b.distance) - (a.profit / a.distance));
      setOpportunities(opps.slice(0, 24));
      setLoading(false);
    }, 50);
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="border border-orange-700 p-3">
        ...
        <button onClick={computeRoutes} className="mt-2 px-3 py-1 border border-orange-700 text-orange-400 hover:bg-orange-950/30 text-[10px] flex items-center gap-1">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-8">
          ...
          <span className="text-orange-600 text-xs ml-2">Scanning nearby markets...</span>
        </div>
        ...
        <div className="space-y-1.5">
          <h3 className="text-orange-500 text-xs font-bold uppercase">Best Trade Opportunities</h3>
...
function findBestCommodity(categories) {
  ...
  for (const cat of categories) {
    const comm = COMMODITIES.find(c => c.category === cat && c.legality === 0);
