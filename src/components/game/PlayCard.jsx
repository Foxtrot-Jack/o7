...
// Renders a per-rarity frame (border glow + rarity stripe) and a manufacturer
// color; foil copies get a gold border, a ✦ marker, and a stamped serial.
import React from 'react';
import { MFR_COLORS, RARITY_ORDER, foilSerial } from '@/lib/cardDeck';
...
export default function PlayCard({ card, faceDown, size = 'md', selected, onClick, dim, foil }) {
  ...
  const isFoil = !!foil;
  return (
    <button onClick={onClick} className={`relative ${w} border-2 bg-black flex flex-col items-center justify-between p-1 leading-none overflow-hidden ${selected ? 'ring-2 ring-cyan-400' : ''} ${dim ? 'opacity-40' : ''} ${r.ring}`} style={{ borderColor: isFoil ? '#ffd24a' : color }}>
      ...
      <div className="text-center">
        ...
        <div className={`text-[6px] ${r.tag}`}>{card.rarity}</div>
      </div>
